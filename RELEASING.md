...
> Zoysh brings the `yo <natural language>` experience from [Fil Pizlo's Yosh](https://github.com/pizlonator/yosh) to zsh as a lightweight plugin. It detects the model loaded in a local OpenAI-compatible server, turns requests into editable commands without executing them, and renders inline answers with terminal-friendly Markdown. It also supports Anthropic, OpenAI, OpenRouter, Kimi, DeepSeek, Qwen, and z.ai, bounded session memory, live config reloads, and optional hosted web search. Install it with a zsh plugin manager or source `zoysh.plugin.zsh`; then review every generated command before running it.
